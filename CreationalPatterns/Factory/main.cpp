...
public:
	CPU(const std::string& model, double frequency, unsigned int number_of_cores, unsigned int l3cache, std::string socket, bool hyper_threading)
	{
		this->model = model;
	...
	virtual const std::string& get_model()const = 0;	//Этот чисто виртуальный метод просто делает класс абстрактным
};
...
{
public:
	Core_i7EE() :CPU("Core-i7 ExtremeEdition 2900K", 4.2, 6, 12, "LGA-2011", true) {}
	~Core_i7EE(){}
	const std::string& get_model()const
	{
		return this->model;
...
enum CPUtype	//Перечисление - это набор именованных ЦЕЛОЧИСЛЕННЫХ констант
{
...
void main()
{
...
#ifdef FACTORY_AS_GLOBAL_FUNCTION_WITH_ENUM
	//class Core_i3 ci3;	ci3.info();
	CPU* my_cpu = CPUfactory(Core_i7EE);
	my_cpu->info();
	//cout << sizeof(CPUtype) << endl;
	int n;
	cout << "Введите количество изделий: "; cin >> n;
	CPU** _cpu = new CPU*[n];
	srand(time(NULL));
	for (int i = 0; i < n; i++)
	{
	...
	CPU* my_cpu = CPUfactory::CreateCPU(CPUfactory::Core_i7);
